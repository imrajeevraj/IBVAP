...
class AnprService:
    ...
    def reset_camera(self, camera_id: str):
        with self._validated_lock:
            self.validated_tracks = {k for k in self.validated_tracks if k[0] != camera_id}
        
    def process_frame(self, frame: np.ndarray, camera_id: str, detections: List[Dict[str, Any]]):
        """
        ...
        vehicle_detections = [d for d in detections if d.get("class") in ["car", "motorcycle", "bus", "truck"] and d.get("track_id")]
        
        # Don't queue up thousands of OCR jobs if they pile up
        if self.queue_depth() > 5:
            logger.debug(f"[{camera_id}] ANPR queue full, skipping frame to prevent OOM")
            return
            
        self.executor.submit(self._run_ocr_async, frame.copy(), camera_id, vehicle_detections)
        
    def _run_ocr_async(self, frame: np.ndarray, camera_id: str, vehicle_detections: List[Dict[str, Any]]):
        import uuid
        import json
        ...
        for det in vehicle_detections:
            ...
            for res in results:
                ...
                if PlateNormalizer.is_valid_format(normalized):
                    logger.debug(f"[{camera_id}] Found plate text '{normalized}' (conf: {conf:.2f}) for track {track_id}")
                    
                    # Add to validator
                    validation_result = self.validator.add_reading(track_id, normalized, conf)
                    
                    if validation_result:
                        best_plate, avg_conf = validation_result
                        
                        plate_bbox = [
                            int(x1 + res["bbox"][0][0]), int(y1 + res["bbox"][0][1]),
                            int(x1 + res["bbox"][2][0]), int(y1 + res["bbox"][2][1])
                        ]
                        ...
                        self._handle_validated_plate(camera_id, track_id, best_plate, avg_conf, forensics)
                        
    def _handle_validated_plate(self, camera_id: str, track_id: str, plate: str, confidence: float, forensics: Dict[str, Any] = None):
        """
        ...
        logger.info(f"[{camera_id}] Plate validated: {plate} (Track: {track_id}, Conf: {confidence:.2f})")
        
        match = watchlist_service.check_plate(plate)
        status = "WATCHLIST MATCH" if match else "CLEAN"
        
        if match:
            logger.warning(f"[{camera_id}] WATCHLIST MATCH FOR PLATE {plate} (Track {track_id}) - Details: {match}")
        ...
        try:
            ...
            event = PlateEvent(
                ...
                validation_state=forensics.get("validation_state") if forensics else None
            )
            db.add(event)
