...
class PlateValidator:
    ...
    def add_reading(self, track_id: str, plate_text: str, confidence: float) -> Optional[Tuple[str, float]]:
        """
    ...
    def _evaluate_consensus(self, track_id: str) -> Optional[Tuple[str, float]]:
        ...
        if best_count / total_reads >= self.consensus_ratio:
            ...
            logger.info(f"Consensus reached for track {track_id}: {best_plate} (conf: {avg_conf:.2f}, hits: {best_count}/{total_reads})")
