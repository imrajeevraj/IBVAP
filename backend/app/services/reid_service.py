...
class ReidService:
    ...
    def match_embedding(self, embedding_vector: list) -> Tuple[Optional[str], str]:
        ...
        db = SessionLocal()
        try:
            stmt_filtered = select(FaceEmbedding).filter(
                FaceEmbedding.embedding.cosine_distance(embedding_vector) < self.match_threshold
            ).order_by(
            ...
            if matched:
                ...
                db.commit()
                return matched.person_name or f"Subject-{matched.id}", matched.watchlist_status
