...
class BehaviorService:
    def __init__(self):
        ...
        self.track_states = {}
        self.db_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="Behavior-DB-Worker")
    ...
    def process_detections(self, camera_id: str, detections: List[Dict[str, Any]], frame=None):
        ...
        for det in detections:
            ...
            if obj_type in ["drone", "airplane"]:
                new_events.append(self._create_event_dict("DRONE_DETECTED", camera_id, None, track_id, "DRONE", None, (x1+x2)/2, y2, conf))
                continue
                
            if obj_type in ["knife", "weapon", "gun"]:
                new_events.append(self._create_event_dict("WEAPON_DETECTED", camera_id, None, track_id, "WEAPON", None, (x1+x2)/2, y2, conf))
                continue
                
            if obj_type in ["backpack", "suitcase", "handbag"]:
                new_events.append(self._create_event_dict("CONTRABAND_DETECTED", camera_id, None, track_id, obj_type.upper(), None, (x1+x2)/2, y2, conf))
                continue
            ...
            if height > 0 and (width / height) > self.CRAWLING_ASPECT_RATIO:
                new_events.append(self._create_event_dict("CRAWLING_DETECTED", camera_id, None, track_id, "PERSON", None, (x1+x2)/2, y2, conf))
            ...
            if track_id:
                ...
                if track_id not in self.track_states[camera_id]:
                    ...
                    if duration > self.LOITERING_TIME_THRESHOLD and "loitering" not in state['alerts']:
                        ...
                        if max_dist < self.LOITERING_DISTANCE_THRESHOLD:
                            ...
                            new_events.append(self._create_event_dict("LOITERING_DETECTED", camera_id, None, track_id, "PERSON", None, center_x, center_y, conf))

        if person_count > self.PLATOON_THRESHOLD:
            new_events.append(self._create_event_dict("PLATOON_DETECTED", camera_id, None, "GROUP", "PERSON", None, 0, 0, 1.0))
    ...
    def _persist_events_async(self, events: List[Dict], camera_id: str, frame):
        ...
        try:
            ...
            for db_event, augmented_evt in persisted_events:
                snapshot_path, _ = evidence_service.capture_event(camera_id, db_event.id, frame)
                if snapshot_path:
    ...
    def _create_event_dict(self, event_type, camera_id, zone_id, track_id, object_type, direction, x, y, confidence):
        return {
