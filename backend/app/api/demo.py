...
router = APIRouter(prefix="/demo", tags=["demo"], dependencies=[Depends(verify_demo_mode)])


def _camera_or_error(camera_id: str):
    frame = camera_manager.get_raw_frame(camera_id)
    if frame is None:
        raise HTTPException(status_code=409, detail="Camera is not ready; wait for its stream to become ONLINE")
    return frame
...
def seed_intrusion(
    ...
    event_data = risk_engine.evaluate_event({
        ...
        "timestamp": datetime.utcnow().replace(hour=2, minute=0, second=0, microsecond=0),
        "confidence": 1.0,
    ...
    db.add(EventAudit(event_id=event.id, action="SIMULATED", actor=user.username, notes="Controlled demo intrusion seeded"))
    db.commit()
    return {"id": f"evt-{event.id}", "message": "Demo intrusion created; clip will be ready in about 7 seconds."}
...
def seed_watchlist_match(
    ...
    db.add(event)
    db.flush()
    db.add(EventAudit(event_id=event.id, action="SIMULATED", actor=user.username, notes="Controlled ANPR watchlist scenario seeded"))
    db.commit()
    return {"plate_event_id": plate_event.id, "event_id": f"evt-{event.id}", "message": "Demo watchlist match created."}
